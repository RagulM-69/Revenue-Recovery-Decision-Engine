...
export default function DecisionsPage() {
  ...
  return (
    <div className="flex-1">
      ...
      <div className="p-8 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="bg-white border border-slate-200 rounded-lg px-4 py-3 shadow-sm">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{item.label}</div>
              <div className={`text-xl font-bold mt-0.5 ${item.color}`}>
        ...
        {/* ── Filter Bar ─────────────────────────────────────────────── */}
        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex flex-col sm:flex-row items-center gap-3">
      ...
      {selected && (
        <div
          className="fixed inset-0 bg-slate-950/30 z-50 flex justify-end"
          onClick={e => { if (e.target === e.currentTarget) setSelected(null); }}
        >
          <div className="bg-white w-full max-w-md h-full shadow-2xl overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
              <div>
                <h3 className="text-sm font-bold text-slate-900">Decision Audit</h3>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={f.label} className="bg-slate-50 border border-slate-100 rounded-lg p-3">
                      <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">{f.label}</div>
                      <div className="text-xs font-semibold text-slate-900 mt-1 break-all">{f.value}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {selected.guardrails_applied.map((check, i) => (
                      ...
                      >
                        ...
                        <div>
                          <div className="text-xs font-semibold text-slate-800">{check.rule}</div>
                          {check.detail && (
