...
import { Search, ChevronDown, ChevronUp, RefreshCw, ShieldAlert, Database, LockKeyhole } from 'lucide-react';
  ...
  return (
    <div className="flex-1">
      ...
      <div className="p-8 space-y-5">
        {/* Search */}
        <div className="bg-white border border-[#E4E9F0] rounded-2xl p-3 shadow-sm">
        ...
        <div className="bg-white border border-[#E4E9F0] rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            <div className="py-16 text-center text-[13px] text-slate-400">Loading audit records…</div>
          ) : filtered.length === 0 ? (
                  ...
                  <tbody className="divide-y divide-[#E4E9F0]">
                    {filtered.slice(0, 200).map(log => (
                      <React.Fragment key={log.log_id}>
                        <tr className="hover:bg-[#F4F6F9]/60 transition-colors">
                          <td className="py-3 px-4 font-mono text-[11px] text-slate-500 whitespace-nowrap">
                            {formatDate(log.created_at)}
                          </td>
                          <td className="py-3 px-4 font-mono text-[11px] text-slate-700 font-semibold">{log.actor}</td>
                          <td className="py-3 px-4">
                            <span className="px-2 py-0.5 bg-slate-100 border border-[#E4E9F0] rounded-lg text-[11px] font-semibold text-slate-700 font-mono">
                              {log.event_type}
                            </span>
                          </td>
